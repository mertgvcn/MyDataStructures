...
public class MyBST {
	...
	private Node insert(Node focus, int data) { // the method running in background recursively; in every insertion, we
												// are redefining the nodes that we passed through
	 ...
	 * 
	 * if (a != null && b != null) { return ((a.data == b.data) && sameTree(a.left,
	 * b.left) && sameTree(a.right, b.right)); }
	 * 
	 * return false; }
	 * 
	 * public int maxDepth(Node focus) { if (focus == null) { return 0; }
	 * 
	 * return 1 + Math.max(maxDepth(focus.left), maxDepth(focus.right));
	 * 
	 * }
	 * 
	 * public int sumRec(Node focus) { if (focus == null) { return 0; }
	 * 
	 * return focus.data + sumRec(focus.left) + sumRec(focus.right); }
	 * 
	 * public void insert(int data) { Node newNode = new Node(data); Node pointer =
	 * root; Node temp = null; //pointer null olduðunda önceki adresi kaybetmemek
	 * için geçici node
	 * 
	 * if (isEmpty()) { root = newNode; } else {
	 * 
	 * while (pointer != null) { //pointer ile eklenecek adresi bulacaðýz temp =
	 * pointer; //pointer null olacaðý için bir önceki deðerini yedekliyoruz
	 * 
	 * if (newNode.data < pointer.data) { pointer = pointer.left; } else if
	 * (newNode.data > pointer.data) { pointer = pointer.right; } }
	 * 
	 * if(newNode.data < temp.data) { //temp eklenecek node'un atasý. temp.left =
	 * newNode; }else { temp.right = newNode; } } }
	 */
}
